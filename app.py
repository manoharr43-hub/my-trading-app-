...
# CONFIG & REFRESH
# =============================
st.set_page_config(page_title="🔥 NSE AI PRO V20 - LIVE FIX", layout="wide")
        ...
        for s in all_stocks:
            ...
            if df_l is not None and not df_l.empty:
                ...
                fig.add_trace(go.Candlestick(
                    x=df_l.index, open=df_l['Open'], high=df_l['High'],
                    low=df_l['Low'], close=df_l['Close'], name=s
                ))
                fig.add_hline(y=price*1.02, line_dash="dash", line_color="green", annotation_text="Target")
                fig.add_hline(y=price*0.99, line_dash="dash", line_color="red", annotation_text="SL")
                st.plotly_chart(fig, use_container_width=True)

    if live_results:
        st.dataframe(pd.DataFrame(live_results), use_container_width=True)
    else:
        st.warning("⚠️ No signals found in LIVE SCAN.")
