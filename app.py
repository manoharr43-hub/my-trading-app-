...
# CONFIG & REFRESH
# =============================
st.set_page_config(page_title="🔥 NSE AI PRO V22 - LIVE+BACKTEST", layout="wide")
st_autorefresh(interval=60000, key="refresh")

IST = pytz.timezone('Asia/Kolkata')
current_time = datetime.now(IST).strftime('%Y-%m-%d %H:%M:%S')

st.title("🚀 NSE AI PRO V22 - ULTIMATE DASHBOARD")
...
with tab2:
    if st.button("📊 RUN BACKTEST", key="backtest_btn"):
        ...
        with st.spinner("Running Backtest..."):
            for s in all_stocks:
                df = get_data(s, period="6mo", interval="1d")
                if df is None:
                    continue
                df = add_indicators(df)
                for i in range(1, len(df)):
                    ...
                    if row['Close'] > row['VWAP'] and row['EMA20'] > row['EMA50']:
                        logs.append({"Date": df.index[i].strftime('%Y-%m-%d'), "Stock": s,
                                     "Price": round(row['Close'], 2), "Signal": "BUY"})
                    elif row['Close'] < row['VWAP'] and row['EMA20'] < row['EMA50']:
                        logs.append({"Date": df.index[i].strftime('%Y-%m-%d'), "Stock": s,
                                     "Price": round(row['Close'], 2), "Signal": "SELL"})
                    elif row['Big_Player']:
                        logs.append({"Date": df.index[i].strftime('%Y-%m-%d'), "Stock": s,
                                     "Price": round(row['Close'], 2), "Signal": "BIG PLAYER"})
                    elif row['Bull_Rev']:
                        logs.append({"Date": df.index[i].strftime('%Y-%m-%d'), "Stock": s,
                                     "Price": round(row['Close'], 2), "Signal": "BUY REVERSAL"})
                    elif row['Bear_Rev']:
                        logs.append({"Date": df.index[i].strftime('%Y-%m-%d'), "Stock": s,
                                     "Price": round(row['Close'], 2), "Signal": "SELL REVERSAL"})
        if logs:
