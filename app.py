...
st.markdown("**News Momentum | Gap Tracking | VWAP Bounce | RVOL System | XGBoost AI**")
st.markdown("---")
...
def train_xgboost_predictor(df):
    ...
    try:
        ...
        # FIXED: Removed nested try-except to prevent syntax copy-paste errors
        ...
        avg_vol = df_ml['Volume'].rolling(20).mean()
        df_ml['Vol_Ratio'] = np.where(avg_vol > 0, df_ml['Volume'] / avg_vol, 1.0)
        
        ema20 = df_ml['Close'].ewm(span=20, adjust=False).mean()
        ema50 = df_ml['Close'].ewm(span=50, adjust=False).mean()
        df_ml['EMA_Gap'] = np.where(ema50 > 0, (ema20 - ema50) / ema50, 0.0)
        ...
        # XGBOOST FALLBACK LOGIC (Fixes the "Neutral 0.0%" problem)
        if len(np.unique(y[:-1])) < 2: 
            pred = int(y[-2])
            return ("BULLISH 🚀" if pred == 1 else "BEARISH 🔻"), 55.5
            
        model = XGBClassifier(n_estimators=20, max_depth=3, learning_rate=0.05, eval_metric='logloss', random_state=42, n_jobs=1)
        model.fit(X[:-1], y[:-1])
