...
# CONFIG
# =============================
st.set_page_config(page_title="🔥 NSE AI PRO V32 HQ", layout="wide")
st.title("🚀 NSE AI PRO V32 HQ - SESSION UPGRADE")
...
if "live" in st.session_state:
    ...
    if not live_df.empty:
        ...
        live_df = session_filter(live_df).sort_values("Time")
        live_df["Time"] = live_df["Time"].dt.strftime("%I:%M %p")
        st.dataframe(live_df, use_container_width=True)
        # 🔥 LIVE Chart
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=live_df["Time"], y=live_df["Price"], mode="lines+markers", name="Signal Price"))
        fig.update_layout(title="LIVE SCAN Signals Chart", template="plotly_dark")
...
if st.button("🔍 RUN BACKTEST"):
    ...
    st.download_button("⬇️ Download CSV", data=result_df.to_csv(index=False).encode(), file_name=f"{bt_stock}_{bt_date}.csv", mime="text/csv")
