...
def predict_trend_ai(close_series):
    if len(close_series) < 30:
        return "NEUTRAL", 0
    y = close_series.tail(30).values
    x = np.arange(len(y))
    slope, intercept = np.polyfit(x, y, 1)
    corr = np.corrcoef(x, y)[0, 1]   # ✅ fixed
    confidence = min(round(abs(corr) * 100, 2), 99)
    if slope > 0 and confidence > 60:
        return "BULLISH 🚀", confidence
    elif slope < 0 and confidence > 60:
...
def detect_structure(df):
    ...
    if close > recent_high:
        return "BOS 📈" if bullish_trend else "CHOCH 🐂"
    elif close < recent_low:
        return "CHOCH 🐻" if bullish_trend else "BOS 📉"
    return "RANGE ➖"
