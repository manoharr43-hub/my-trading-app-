...
with tab1:
    if run_button or auto_refresh:
        ...
        if results:
            ...
            if buy_count > 0: st.toast(f"🔥 V11.11 ACTION ALERT: {buy_count} STRONG BUY Signals Generated!", icon='⚡')

    # DISPLAY BLOCK
    if not st.session_state.v11_master_data.empty:
        final_df = st.session_state.v11_master_data
        
        st.markdown("### 🏆 Top Institutional Breakouts (V11.11 Master Picks)")
        ...
        with col2:
            st.download_button(
                ...
                data=excel_data,
                file_name="NSE_AI_PRO_Master_Report_V11.11.xlsx",
