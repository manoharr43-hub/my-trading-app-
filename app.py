...
def analyze_data(df):
    ...
    vol = df['Volume']; avg_vol = vol.rolling(20).mean()
    if pd.isna(avg_vol.iloc[-1]): return None
...
def breakout_engine(df, stock):
    ...
    high = opening['High'].max(); low = opening['Low'].min()
...
if st.button("🔍 START LIVE SCANNER (9:15–3:30)"):
    ...
    for s in stocks:
        try:
            ...
            res = analyze_data(df)
            if res:
                live_results.append({"Stock":s,"Price":round(df['Close'].iloc[-1],2),
                                     "Trend":res[0],"Signal":res[1],
