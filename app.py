...
# 1. CONFIG
# =============================
st.set_page_config(page_title="NSE Pro Intraday Dashboard", layout="wide")
st_autorefresh(interval=20000, key="refresh")  # refresh every 20 sec
...
# 4. INTERFACE
# =============================
st.title("🛡️ NSE Pro Intraday Dashboard")
...
with st.sidebar:
    ...
    interval = st.selectbox("Select Candle Interval", ["1m","2m","5m","15m","30m"], index=2)

# =============================
# 5. SCANNER FUNCTION
# =============================
def process_scanner(tickers, interval):
    ...
    for t in tickers:
        try:
            ...
            # Status
            status="NORMAL"
            if ltp>=res: status="🚀 BREAKOUT" if vol_ratio>1.2 else "⚠️ FAKE (Low Vol)"
            elif ltp<=sup: status="📉 BREAKDOWN" if vol_ratio>1.2 else "⚠️ FAKE (Low Vol)"
            # Signal
...
# 6. DISPLAY & DASHBOARD
# =============================
res_df = process_scanner(sectors[sector_name], interval)

if not res_df.empty:
    res_df = res_df[res_df['Trend'].isin(trend_filter)]
    
    # Summary
    buy_count = (res_df['Signal']=="BUY").sum()
    sell_count = (res_df['Signal']=="SELL").sum()
    wait_count = (res_df['Signal']=="WAIT").sum()
    
    st.subheader("📊 Signals Summary")
    st.markdown(f"**Strong BUY:** {buy_count}  |  **Strong SELL:** {sell_count}  |  **WAIT:** {wait_count}")
    ...
    def style_table(row):
        ...
        if abs(row.LTP-row.Support)/row.Support<0.002: styles[ltp_idx]='color:#00ffff;font-weight:bold'
        elif abs(row.LTP-row.Resistance)/row.Resistance<0.002: styles[ltp_idx]='color:#ff3131;font-weight:bold'
        return styles
